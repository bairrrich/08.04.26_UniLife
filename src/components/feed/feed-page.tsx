...
export default function FeedPage() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        setPosts(data.data)
        const liked = new Set<string>(data.data.filter((p: FeedPost) => p._count.likes > 0).map((p: FeedPost) => p.id))
        setLikedPosts(liked)
      }
    } catch (err) {
  ...
  const handleToggleLike = (postId: string) => {
    ...
    setPosts((prev) => prev.map((p) => p.id === postId
      ? { ...p, _count: { likes: likedPosts.has(postId) ? p._count.likes - 1 : p._count.likes + 1 } } : p
    ))
    if (willLike) {
      setLikeAnimating((prev) => new Set(prev).add(postId))
      setTimeout(() => setLikeAnimating((prev) => { const next = new Set(prev); next.delete(postId); return next }), 300)
    }
  }
  ...
  const handleToggleBookmark = (postId: string) => {
    setBookmarkedPosts((prev) => {
      ...
      if (next.has(postId)) { next.delete(postId); toast.info('Запись удалена из сохранённых') }
      else { next.add(postId); toast.success('Запись сохранена') }
      return next
    })
  }
  ...
  const handleShare = (post: FeedPost) => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(() => toast.success('Ссылка скопирована в буфер обмена')).catch(() => toast.error('Не удалось скопировать ссылку'))
    } else {
  ...
  const handleCommentSubmit = async (postId: string) => {
    ...
    toast.dismiss(); setSendingComment((prev) => new Set(prev).add(postId))

    const optimisticComment: FeedComment = {
      id: 'opt_' + Date.now(), content: text, createdAt: new Date().toISOString(),
    ...
    try {
      ...
      if (json && json.success) {
        ...
        setPosts((prev) => prev.map((p) => p.id === postId
          ? { ...p, comments: p.comments.map((c) => c.id === optimisticComment.id ? json.data : c) } : p
        ))
      } else {
        toast.error('Ошибка при добавлении комментария')
        setPosts((prev) => prev.map((p) => p.id === postId
          ? { ...p, comments: p.comments.filter((c) => c.id !== optimisticComment.id) } : p
        ))
      }
    } catch (err) {
      ...
      setSendingComment((prev) => { const next = new Set(prev); next.delete(postId); return next })
    }
  }

  const handleCommentKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, postId: string) => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleCommentSubmit(postId) }
  }
  ...
  const toggleCommentSection = (postId: string) => {
    setShowCommentSection((prev) => { const next = new Set(prev); if (next.has(postId)) next.delete(postId); else next.add(postId); return next })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (json && json.success) {
        toast.success('Запись опубликована'); setDialogOpen(false)
        setFormEntityType('diary'); setFormCaption(''); fetchPosts()
      } else {
